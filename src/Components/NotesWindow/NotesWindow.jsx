...
const NotesWindow = ({ list, setList, selected }) => {
  ...
  const getCurrentTime = () => {
    ...
    const formattedTime = currentTime.replace(/am|pm/gi, (match) =>
      match.toUpperCase()
    );
    ...
    <div className={Styles.window}>
      <div className={Styles.title_bar}>
        <div
          ...
          style={{ backgroundColor: list[selected].color }}
        >
          {list[selected].abbreviation}
        </div>
        <div className={Styles.group_title}>{list[selected].title}</div>
      </div>
      <div className={Styles.notes_date_time_content}>
        {selected > -1
          ? selectedNote.map((item) => (
              <NoteList list={list} item={item} key={item.id} />
            ))
          : ""}
      </div>
      <div className={Styles.text_area_container}>
        ...
        <div className={Styles.text_area_button_img}>
          <img
            src={ButtonImage}
            className={Styles.text_area_button_img}
            alt="Enter-Button"
            onClick={handleClick}
            ref={buttonRef}
          />
